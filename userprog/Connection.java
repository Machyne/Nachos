...
public class Connection extends OpenFile {
	 ...
	 * The current and expected sequence numbers are set to 0
	 */
	public Connection(NetMessage SYN, Runnable closer){
	...
	public void open() {
		...
		NetMessage SYN;
		try{ SYN = new NetMessage(dstNode,dstPort,srcNode,srcPort,SYN_FLAGS,0,new byte[0]);}
	...
	/**
	 * Initiates the 2-way close handshake by sending a FIN
	 */
	public void close() {
		if (!isConnected) { return; }
		NetMessage STP;
		try{ STP = new NetMessage(dstNode,dstPort,srcNode,srcPort,STP_FLAGS,0,new byte[0]);}
		catch (Exception e) { System.out.println(e); closer.run(); return;}
		PO.send(STP);
	...
	public int read(byte[] buf, int offset, int length) {
		...
		while (read<toRead) {
			NetMessage nm = PO.receive(srcPort);
			if (nm == null) { break; }
			
			if (nm.getFlags()[1]) { //this is a stop
				NetMessage FIN; 
				try{ FIN = new NetMessage(dstNode,dstPort,srcNode,srcPort,FIN_FLAGS,0,new byte[0]);}
	...
	public void recievedSyn(){
		NetMessage ACK;
		try{ ACK = new NetMessage(dstNode,dstPort,srcNode,srcPort,ACK_FLAGS,0,new byte[0]);}
		catch (Exception e) { System.out.println(e); closer.run(); return;}
		PO.send(ACK);
		isConnected = true;
	}
	...
	public int write(byte[] buf, int offset, int length) {
		...
		while(written!=toWrite){
			...
			NetMessage nm;
			try{ nm = new NetMessage(dstNode,dstPort,srcNode,srcPort,STD_FLAGS,currSeqno,contents);}
			catch (Exception e) { System.out.println(e); closer.run(); return -1;}
	...
	public static final int maxContentSize = Packet.maxContentsLength - NetMessage.headerLength;
}
