...
public class PostOffice {
    ...
    public PostOffice() {
		...
		queues = new SynchList2[NetMessage.portLimit];
    ...
    public NetMessage receive(int port) {
		...
		NetMessage mail = (NetMessage) queues[port].removeFirst();
     ...
     * @return	the message received
     */
    public NetMessage bReceive(int port) {
		Lib.assertTrue(port >= 0 && port < queues.length);
		
		Lib.debug(dbgNet, "waiting for mail on port " + port);
		
		
		NetMessage mail = null;
		
		lock.acquire();
		while (mail==null) {
			cond.sleep();
			mail = (NetMessage) queues[port].removeFirst();
		}
		lock.release();
    ...
    private void postalSendery() {
		while(true){
			NetMessage mail = (NetMessage) sendList.removeFirst();
